...
$publishing_connection = file_get_contents( $root . '/admin/partials/publishing-connection-status.php' );
check_client_safety_case( 'settings connection status does not expose employee controls', str_contains( $settings, "'/publishing-connection-status.php'" ) && str_contains( $publishing_connection, 'No secret or connection settings need to be entered here.' ) && ! str_contains( $publishing_connection, '<input' ) && ! str_contains( $publishing_connection, '<button' ) && ! str_contains( $publishing_connection, "['mode']" ) );
