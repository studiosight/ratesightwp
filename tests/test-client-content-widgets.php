...
check_client_content_widgets_case( 'client page hides identity and authentication internals', ! preg_match( '/wp_ratesight_(code_id|campaign_id|domain_id)|Ratesight ID|Campaign ID|Domain ID|\bOID\b|authenticates/i', $widgets ) );
check_client_content_widgets_case( 'identity options keep names but use a separate settings group', preg_match( "/'code_id'\s*=>.*'name'\s*=>\s*'wp_ratesight_code_id'.*'group'\s*=>\s*'widget_identity'/", $options ) && preg_match( "/'campaign_id'\s*=>.*'group'\s*=>\s*'widget_identity'/", $options ) && preg_match( "/'domain_id'\s*=>.*'group'\s*=>\s*'widget_identity'/", $options ) );
