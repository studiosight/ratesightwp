...
check_client_navigation_case( 'Activity Log exposes sanitized arrival and authentication diagnostics', str_contains( $logs, 'Webhook Request Diagnostics' ) && str_contains( $logs, '<details class="rs-card"' ) && str_contains( $logs, 'ratesight_auth_audit' ) && str_contains( $logs, 'request bodies, signatures, and secrets are never shown' ) );
check_client_navigation_case( 'legacy Activity Log remains above collapsed technical diagnostics', strpos( $logs, '<h2>Activity Log</h2>' ) < strpos( $logs, '<details class="rs-card"' ) );
