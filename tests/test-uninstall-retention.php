...
check_uninstall_case( 'shared-state inventory includes tables, CPT, meta, nonce prefixes, transients, and cron', count( $inventory['tables'] ) === 8 && $inventory['postTypes'] === array( 'ratesight_page' ) && count( $inventory['postMeta'] ) === 16 && $inventory['userMeta'] === array( 'ratesight_wizard_dismissed' ) && $inventory['optionPrefixes'] === array( 'ratesight_auth_nonce_', 'ratesight_pairing_nonce_' ) && $inventory['transientPrefixes'] === array( 'rs_', 'ratesight_' ) && in_array( 'ratesight_prune_auth_nonces', $inventory['cronHooks'], true ) );
