...
check_ownership_case( 'Connections entrypoint renders the dashboard-owned surface before suppressing legacy controls', str_contains( $connections_ui, "require __DIR__ . '/tab-connections-dashboard.php';" ) && str_contains( $connections_ui, 'if ( false ) : // Legacy provider controls are retained in source for rollback, but no longer rendered.' ) );
$dashboard_connections_ui = file_get_contents( __DIR__ . '/../admin/partials/tab-connections-dashboard.php' );
check_ownership_case( 'dashboard-owned surface is read-only and client safe', str_contains( $dashboard_connections_ui, 'data-ratesight-owner="dashboard"' ) && str_contains( $dashboard_connections_ui, 'There is nothing you need to connect in WordPress.') && ! str_contains( $dashboard_connections_ui, 'Open Dashboard Connections' ) && ! str_contains( $dashboard_connections_ui, 'Legacy Provider State' ) && ! preg_match( '/rs-(?:load|lock|disconnect|sync)-(?:gsc|gbp|bing)/', $dashboard_connections_ui ) && ! str_contains( $dashboard_connections_ui, 'name="ratesight_deepseek_api_key"' ) );
