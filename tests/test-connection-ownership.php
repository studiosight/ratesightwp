...
check_ownership_case( 'dashboard-owned surface has one provider destination and no provider mutation controls', str_contains( $dashboard_connections_ui, 'data-ratesight-owner="dashboard"' ) && str_contains( $dashboard_connections_ui, 'Open Dashboard Connections' ) && str_contains( $dashboard_connections_ui, 'They cannot be created, changed, or disconnected from this plugin.') && ! preg_match( '/rs-(?:load|lock|disconnect|sync)-(?:gsc|gbp|bing)/', $dashboard_connections_ui ) && ! str_contains( $dashboard_connections_ui, 'name="ratesight_deepseek_api_key"' ) );
