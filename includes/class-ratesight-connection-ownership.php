...
class Ratesight_Connection_Ownership {
		...
		foreach ( $definitions as $path => $methods ) {
			foreach ( $methods as $method ) {
				$result[] = array(
					...
					'source' => str_contains( $path, 'performance-snapshot' ) ? 'includes/class-ratesight-performance-snapshot.php' : ( $path === '/pair' ? 'includes/class-ratesight-pairing.php' : ( $path === '/enrollment-challenge' || $path === '/claim' ? 'includes/class-ratesight-enrollment.php' : ( $path === '/plugin-update' ? 'includes/class-ratesight-release-update.php' : ( str_contains( $path, 'related-links' ) ? 'includes/class-ratesight-related-links.php' : ( str_contains( $path, '/page' ) ? 'includes/class-ratesight-page-api.php' : ( str_contains( $path, 'trash-' ) || str_contains( $path, 'restore-' ) ? 'includes/class-ratesight-page-lifecycle.php' : 'includes/class-ratesight-webhook-handler.php' ) ) ) ) ) ),
	...
	private static function provider_operations(): array {
		$definitions = array(
			...
			'includes/class-ratesight-enrollment.php::post_to_dashboard:wp_remote_post#1' => 'dashboard_enrollment_write',
